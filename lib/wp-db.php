...
<p>If you don\'t know how to setup a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
	...
	function prepare($query = null) { // ( $query, *$args )
		...
		$args = func_get_args();
		array_shift($args);
		//Nếu args đã được thông qua như là một mảng (như trong vsprintf), di chuyển chúng lên
	...
	function print_error($str = '') {
		...
		print "<div id='error'>
		<p class='wpdberror'> <strong>Lỗi:</strong> Số chứng minh nhân dân và địa chỉ Email của bạn đã có người sử dụng<br />
	...
	/**
	 * Lấy một cột từ cơ sở dữ liệu.
*
* Thực hiện một truy vấn SQL và trả về cột từ kết quả SQL.
* Nếu kết quả SQL chứa nhiều cột, chức năng này trả về cột được chỉ định.
* Nếu truy vấn $ là null, hàm này trả về cột được chỉ định từ kết quả SQL trước đó.
	 *
	 * @since 0.71
	 *
	 * @param string|null $query SQL query.  If null, use the result from the previous query.
	...
	function get_results($query = null, $output = OBJECT) {
		...
		if ( $output == OBJECT ) {
			// Trả lại một mảng các đối tượng hàng
			return $this->last_result;
		} elseif ( $output == OBJECT_K ) {
			// Trả lại một mảng các đối tượng hàng với các phím từ cột 1
			// (Bản sao bị hủy)
			...
			// Trả về một số nguyên-keyed mảng của ...
			if ( $this->last_result ) {
				$i = 0;
	 ...
	 * @param False | string | resource $ dbh_or_table (không thực hiện) Cơ sở dữ liệu nào để kiểm tra. False = cơ sở dữ liệu được lựa chọn hiện tại, string = cơ sở dữ liệu có chứa bảng quy định, resource = cơ sở dữ liệu tương ứng với tài nguyên mysql được chỉ định.
	 * @return bool
	 */
...
if ( ! isset($wpdb) ) {
	 ...
	 * @since 0.71
	 */
	$wpdb = new wpdb('root', '', 'dienthoai', 'localhost');
    //$wpdb = new wpdb('vieclamd_mobi', 'tamta', 'vieclamd_mobifone', 'localhost');
}
?>